...
test("el panel filtra por identidad autenticada, calcula la comisión en vivo (15% de la primera venta) y expone solo leads y comisiones propias", async t => {
  ...
  t.mock.method(SellerSale, "find", filter => {
    // sellerID es el mismo campo que usa un seller normal: el influencer no
    // tiene un campo de atribución aparte.
    assert.deepEqual(filter, { sellerID: "influencer-a" });
    return query([
      { userID: "lead-a", subscriptionDate: new Date(1000), amount: 10000, refundedAmount: 2000, paymentStatus: "approved" },
      { userID: "lead-a", subscriptionDate: new Date(2000), amount: 10000 },
    ]);
  });
  const result = await getInfluencerOverview({ _id: "influencer-a", name: "Ana", code: "ANA-001" });
  // Primera venta: (10000 - 2000) * 15% = 1200. La segunda (renovación) no
  // vuelve a generar comisión, sin importar su propio amount.
  ...
  assert.doesNotMatch(JSON.stringify(result), /privado|refundedAmount/);
});

test("la comisión neta es siempre 15% de amount, calculado en vivo, y respeta reembolsos/contracargos", () => {
  const sale = { amount: 12345.67 };
  ...
  assert.equal(netInfluencerCommission({ ...sale, refundedAmount: 345.67 }), 1800);
});
